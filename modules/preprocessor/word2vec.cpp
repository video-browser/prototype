...
public:
    ...
    void initUnigramTable() {
        ...
        for (a = 0; a < table_size; a++) {
            ...
            if (it == vocab.wordTable.end()) { it = prevIt; }
        }
    }
...
public:
    void initNet() {
        ...
        for (const auto &n:vocab.wordTable) {
    ...
    int getVocabSize() { return vocab.getVocabSize(); }
    ...
    void saveResult() {
        ...
        if (classes == 0) {
            for (const auto &it:vocab.wordTable) {
...
void trainModelThread(Word2Vec &w2v, int threadId) {
    ...
    while (true) {
        ...
        if (inFile.eof() || (wordCount > w2v.totalWordCount / w2v.numThreads)) {
            w2v.wordCountActual += wordCount - lastWordCount;
            localIterTimes--;
            if (w2v.debugMode > 1) { cout << "Local Iterator: " << localIterTimes << endl; }

            if (localIterTimes == 0) { break; }
            wordCount = 0;
            lastWordCount = 0;
            inFile.seekg(w2v.trainFileSize / (long long) w2v.numThreads * (long long) threadId);
        }
        ...
        if (word == w2v.vocab.endIter()) { continue; }
        ...
        if (w2v.cbow) {
            ...
            for (long long i = realWindowSize; i < w2v.window * 2 + 1 - realWindowSize; i++) {
                if (i != w2v.window) {
                    ...
                    if (j < 0 || j >= sentence.size()) { continue; }
                    lastWord = sentence[j];
                    if (lastWord == w2v.vocab.endIter()) { continue; }
                    /// Index of word in vector table
                    auto index = lastWord->second.currentNode;

                    // Calculate sum of word vector in window
                    for (long long k = 0; k < w2v.layer1Size; k++) { neu1[k] += w2v.syn0[index][k]; }
                    localWordCount++;
                }
            }
...
void hierarchicalSoftmax(Word2Vec &w2v, const vector<double> &neu1, vector<double> &neu1e,
    ...
    for (size_t i = 0; i < word->second.code.size(); i++) {
        ...
        for (int j = 0; j < w2v.layer1Size; j++) { f += w2v.syn1[index][j] * neu1[j]; }
        if (f <= -w2v.MAX_EXP or f >= w2v.MAX_EXP) { continue; }
        else { f = w2v.expTable[(int) ((f + w2v.MAX_EXP) * (w2v.EXP_TABLE_SIZE / w2v.MAX_EXP / 2))]; }

        double g = (1 - word->second.code[i] - f) * w2v.alpha; ///< gradient*learning rate
        // Propagate errors output -> hidden
        for (int j = 0; j < w2v.layer1Size; j++) { neu1e[j] += g * w2v.syn1[index][j]; }
        // Learn weights hidden -> output
        for (int j = 0; j < w2v.layer1Size; j++) { w2v.syn1[index][j] += g * neu1[j]; }
    }
}
...
void negativeSampling(Word2Vec &w2v,
    ...
    for (int i = 0; i < w2v.ns + 1; i++) {
        ...
        double f = 0;
        for (int j = 0; j < w2v.layer1Size; j++) { f += neu1[j] * w2v.syn1neg[index][j]; }
        double g;

        if (f > w2v.MAX_EXP) { g = (label - 1) * w2v.alpha; }
        else if (f < -w2v.MAX_EXP) { g = (label - 0) * w2v.alpha; }
        else {
            g = (label -
        ...
        // Propagate errors output -> hidden
        for (int j = 0; j < w2v.layer1Size; j++) { neu1e[j] += g * w2v.syn1neg[index][j]; }
        // Learn weights hidden -> output
        for (int j = 0; j < w2v.layer1Size; j++) { w2v.syn1neg[index][j] += g * neu1[j]; }
    ...
    for (long long a = realWindowSize; a < w2v.window * 2 + 1 - realWindowSize; a++) {
        if (a != w2v.window) {
            ...
            for (c = 0; c < w2v.layer1Size; c++) { w2v.syn0[l][c] += neu1e[c]; }
        }
    }
}
...
void trainModel(Word2Vec &w2v) {
    ...
    if (w2v.ns > 0) { w2v.initUnigramTable(); }
    w2v.start = clock();

    vector<thread> threadPool;

    threadPool.reserve(w2v.numThreads);
...
void findSimiliarWord(Word2Vec &w2v, const string &word) {
    ...
    for (const auto &candidate:w2v.vocab.wordTable) {
        ...
        result.emplace_back(candidate.second.word, score);
    }
    auto comp = [](const pair<string, double> &lhs, const pair<string, double> &rhs) {
